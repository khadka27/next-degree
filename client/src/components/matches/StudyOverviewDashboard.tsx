...
import { Bell, Edit3, ChevronRight, FileText, Search, Target, Bookmark, Sparkles, MapPin, CheckCircle2, AlertTriangle, IndianRupee } from 'lucide-react';
...
export function StudyOverviewDashboard({
  ...
  return (
    <div className="bg-[#f8fafc] min-h-screen text-slate-900 pb-20">
      {/* Top Header */}
      <div className="pt-10 px-5 pb-6 bg-white shadow-sm flex items-start justify-between rounded-b-[32px]">
        <div>
      ...
      <div className="px-5 mt-6 space-y-6">
        ...
        {/* Engine Metric Cards (Horizontal Scroll) */}
        <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar -mx-5 px-5 snap-x">
          {/* Cost Card */}
          <div className="min-w-[260px] bg-white border border-slate-200 rounded-3xl p-5 shadow-sm snap-center flex flex-col justify-between border-b-4 border-b-emerald-500">
             <div>
                <div className="flex items-center gap-2 mb-2">
                   <div className="w-6 h-6 rounded-full bg-slate-900 text-white flex items-center justify-center">
                     <IndianRupee className="w-3.5 h-3.5" />
                   </div>
                   <h3 className="font-bold text-slate-800 text-sm">Estimated Cost</h3>
                </div>
                <div className="mt-4">
                  <span className="text-[22px] font-black tracking-tight text-slate-900">
                     NPR {(totalYear1Npr / 100000).toFixed(1)}L
                  </span>
                  <span className="text-slate-500 font-bold text-xs ml-1">/ year</span>
                </div>
                <div className="mt-3">
                  <span className={`inline-block px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full ${costBand.badgeClass}`}>
                    • {costBand.label}
                  </span>
                </div>
                <p className="text-xs font-semibold text-slate-500 mt-3">Tuition + Living</p>
             </div>
             <button onClick={onAdvanceToCost} className="mt-6 w-full py-3 bg-[#63b931] hover:bg-[#529e28] text-white font-black text-sm rounded-xl transition-colors shadow-md shadow-[#63b931]/20">
               View Breakdown
             </button>
          </div>

          {/* Admission Card */}
          <div className="min-w-[260px] bg-white border border-slate-200 rounded-3xl p-5 shadow-sm snap-center flex flex-col justify-between border-b-4 border-b-blue-500">
             <div>
                <div className="flex items-center gap-2 mb-2">
        ...
        <div className="relative bg-gradient-to-br from-[#fdfbf6] to-[#f4ead9] border border-[#e8dcc4] rounded-3xl p-6 overflow-hidden shadow-sm">
          ...
          <div className="absolute right-[-20px] bottom-0 w-44 h-44 opacity-90 pointer-events-none">
            <Image src="/study.png" alt="Improve" fill className="object-contain" />
          </div>
        </div>
